...
function emacs_server_wait(hexo, count) {
	...
	const emacsclient = config.org.emacsclient;
        ...
        proc.on('exit', function(code) {
            if (code != 0) {
		if (lefted <= 0) {
		    error("Can not waiting the emacs server to started, please check the emacs server")
		    reject(code);
		    return;
		}
		
                setTimeout(() => {
		    debug("Waiting emacs starting...")
...
function emacs_process(hexo, data, count) {
	...
	proc.on('exit', (code, signal) => {
	    if (code != 0) {
		if (lefted <= 0) {
		    error(`Can not waiting the emacs server to render, please check the emacs server or the file: ${data.path}`)
		    reject(code);
		    return;
		}
		
		setTimeout(() => {
		    debug("Waiting for emacs...")
                    resolve(emacs_process(hexo, data, lefted-1));
                }, 5000);
		return;
	    }

	    debug(`DONE: ${data.path}`)
